...
def build_cs2_profile_embeds(
    ...
    embed.add_field(name=":chart_with_upwards_trend: RATING", value=_signed(avg_rating), inline=True)
    embed.add_field(name=":mountain_snow: PEAK RATING", value=_peak_label(ranks), inline=True)
    embed.add_field(name=":trophy: WINRATE", value=f"{ct_win}% / {t_win}%", inline=True)

    embed.add_field(name=":dart: AIM", value=_fmt(rating.get("aim")), inline=True)
    embed.add_field(name=":round_pushpin: POSITIONING", value=_fmt(rating.get("positioning")), inline=True)
    embed.add_field(name=":triangular_ruler: PREAIM", value=f"{_fmt(stats.get('preaim'), 2)}\u00b0", inline=True)

    embed.add_field(name=":crossed_swords: OPENING", value=_signed(rating.get("opening")), inline=True)
    embed.add_field(name=":ice_cube: CLUTCH", value=_signed(rating.get("clutch")), inline=True)
    embed.add_field(name=":skull: KD", value=kd_ratio, inline=True)

    embed.add_field(name=":bomb: UTILITY", value=_fmt(rating.get("utility")), inline=True)
    embed.add_field(name=":tada: PARTY", value=party_score, inline=True)
    embed.add_field(name=":boom: AVG HE DMG", value=he_dmg, inline=True)

    embed.add_field(name=":video_game: MATCHES", value=f"__{total_matches}__ / {ban_pct}%", inline=True)
    embed.add_field(name=":zap: TIME TO DMG", value=f"{round(stats.get('reaction_time_ms', 0))}ms", inline=True)
    embed.add_field(name=":no_entry_sign: BANNED MATES", value=banned_mates_pct, inline=True)
